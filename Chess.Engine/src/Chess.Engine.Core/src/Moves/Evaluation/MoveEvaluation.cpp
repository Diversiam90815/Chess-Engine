...
int MoveEvaluation::getMediumEvaluation(const PossibleMove &move, PlayerColor player)
{
	...
	score += evaluateOpeningPrinciples(move, player); // Opening game principles
...
int MoveEvaluation::evaluateMaterialGain(const PossibleMove &move)
{
	...
	// PieceType capturedPieceType = getPieceTypeFromPosition(move.end);
	// PieceType movingPieceType	= getPieceTypeFromPosition(move.start);
...
int MoveEvaluation::evaluateThreatLevel(const PossibleMove &move, PlayerColor player)
{
	...
	//{
	//	PlayerColor pieceColor = PlayerColor::None;
	//	//PieceType	pieceType  = PieceType::DefaultType;

	//	//pieceType			   = getPieceTypeFromPosition(square);
	//	pieceColor			   = getPieceColorFromPosition(square);
...
{
	int score = 0;
	// PieceType type	= getPieceTypeFromPosition(move.start);
...
int MoveEvaluation::evaluateOpeningPrinciples(const PossibleMove &move, PlayerColor player)
{
...
std::vector<Position> MoveEvaluation::calculateThreatsOnBoard(PlayerColor opponent, PlayerColor player)
{
	std::vector<Position> threats;
...
int MoveEvaluation::countAttackers(const Position &target, PlayerColor attackerPlayer) const
{
	...
	for (int y = 0; y < BOARD_SIZE; ++y)
	{
		for (int x = 0; x < BOARD_SIZE; ++x)
		{
			...
			PlayerColor	   pieceColor = getPieceColorFromPosition(pos);
...
bool MoveEvaluation::isDevelopmentMove(const PossibleMove &move, PlayerColor player) const
{
...
bool MoveEvaluation::isPrematureQueenMove(const PossibleMove &move, PlayerColor player) const
{
	// PieceType piece = getPieceTypeFromPosition(move.start, lightBoard);
...
bool MoveEvaluation::controlsCenterSquares(const PossibleMove &move, PlayerColor player) const
{
...
//
// PieceType MoveEvaluation::getPieceTypeFromPosition(const Position &pos) const
//{
//	if (lightBoard)
//	{
