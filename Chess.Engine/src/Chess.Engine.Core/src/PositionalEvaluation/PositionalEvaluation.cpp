...
PositionalEvaluation::PositionalEvaluation(MoveEvaluation &moveEvaluation) : mMoveEvaluation(moveEvaluation) {}


int PositionalEvaluation::evaluatePosition(PlayerColor player)
{
	return evaluatePositionDetailed(player).getTotalScore();
}


PositionalEvaluationResult PositionalEvaluation::evaluatePositionDetailed(PlayerColor player)
{
	...
	result.materialScore			   = static_cast<int>(evaluateMaterial(player) * weights.materialWeight);
	result.positionalScore			   = static_cast<int>(evaluatePositionalAdvantage(player) * weights.positionalWeight);
	result.kingSafetyScore			   = static_cast<int>(evaluateKingSafety(player) * weights.kingSafetyWeight);
	result.mobilityScore			   = static_cast<int>(evaluateMobility(player) * weights.mobilityWeight);
	result.tacticalScore			   = static_cast<int>(evaluateTacticalOpportunities(player) * weights.tacticalWeight);
	result.structuralScore			   = static_cast<int>(evaluatePawnStructure(player) * weights.structuralWeight);

	return result;
}


GamePhase PositionalEvaluation::determineGamePhase() const
{
	return mMoveEvaluation.determineGamePhase();
}
...
int PositionalEvaluation::evaluateMaterial(PlayerColor player)
{
...
int PositionalEvaluation::evaluatePositionalAdvantage(PlayerColor player)
{
	int score = 0;
...
int PositionalEvaluation::evaluateKingSafety(PlayerColor player)
{
	int score = 0;
	// GamePhase	phase	 = determineGamePhase();
	// PlayerColor opponent = getOpponent(player);
	...
	//	score += mMoveEvaluation->getPositionValue(PieceType::King, playerKingPos, player);
	//	score -= mMoveEvaluation->getPositionValue(PieceType::King, opponentKingPos, opponent);
	//}
...
int PositionalEvaluation::evaluateMobility(PlayerColor player)
{
...
int PositionalEvaluation::evaluateTacticalOpportunities(PlayerColor player)
{
	return evaluateBestMovesOpportunity(player, 3) - evaluateBestMovesOpportunity(getOpponent(player), 3);
}


int PositionalEvaluation::evaluatePawnStructure(PlayerColor player)
{
	int score = 0;
	// PlayerColor opponent	  = getOpponent(player);
	...
	// score += evaluatePawnMajority(player);
	// score += evaluatePawnChains(player);
...
int PositionalEvaluation::evaluateBestMovesOpportunity(PlayerColor player, int maxMoves)
{
...
bool PositionalEvaluation::hasPawnSupport(const Position &pawnPos, PlayerColor player) const
{
...
int PositionalEvaluation::evaluatePawnMajority(PlayerColor player) const
{
	int score = 0;
	// PlayerColor opponent			   = getOpponent(player);
...
int PositionalEvaluation::evaluatePawnChains(PlayerColor player) const
{
	int score = 0;
	// auto playerPawns = board.getPiecePositions(player, PieceType::Pawn);
